...
def find_error(y_pred,y):
    error = 0
    N = len(y_pred)
    cross_mat = np.array([[0,0],[0,0]])
    for i,a in enumerate(y_pred):
        if a!=y[i]:
            error+=1
        cross_mat[int(a)][y[i]]+=1

    pre = cross_mat[1][1]/(cross_mat[1][1]+cross_mat[1][0])
    rec = cross_mat[1][1]/(cross_mat[1][1]+cross_mat[0][1])
    f1 = 2*(1/(1/pre+1/rec))
...
def create_feature(name,d):
    ...
    if 0<=last_1<26 and 0<=last_2<26:
        ...
        if 0<=fst<26:
            x[26+26*26+fst] +=1
        # first 2 characters
        if 0<=fst<26 and 0<=sec<26:
            # freq of alphabets, last 2, last 1, first 2, first 1
            x[26+26*26+26+sec*26 +fst] +=1
...
if __name__ == '__main__':
    X_train,y_train,X_test,y_test = get_train_test('names_dataset_v1.pkl')
    d = len(X_train[0])
    ...
    with open('cond_prob.pkl','wb') as f:
        pkl.dump({'P_B':P_B,'P_G':P_G},f)
